...
RC createPageFile (char *fileName) {
	...
	condition((filept != NULL),{
		...
		condition((fwrite(emptyPage, sizeof(char), PAGE_SIZE,filept) >= PAGE_SIZE),
		{
...
RC openPageFile (char *fileName, SM_FileHandle *fHandle) {
	...
	else
	{
	    ...
	    //(*fHandle).totalNumPages = (ftell(filept) + 1) / PAGE_SIZE;
			condition(lesser((fstat(fileno(filept), &fi)),0),
				{
					return RC_ERROR;
				})
...
RC closePageFile (SM_FileHandle *fHandle) {
	condition((filept != NULL),{filept = NULL;})
	return RC_OK;
}
...
RC readBlock (int pageNum, SM_FileHandle *fHandle, SM_PageHandle memPage) {
  ...
  // If else condition to check the block size
		condition((fread(memPage, sizeof(char), PAGE_SIZE, filept) < PAGE_SIZE),{return RC_ERROR;})
	})
	else
	{
		return RC_READ_NON_EXISTING_PAGE;
...
RC readFirstBlock (SM_FileHandle *fHandle, SM_PageHandle memPage) {
	...
	condition(doEq(filept,NULL),{return RC_FILE_NOT_FOUND;})
	for(int i=0;i<PAGE_SIZE;i++)
	{
		char bit = fgetc(filept);
		condition(feof(filept),{break;})
		else
		{
			memPage[i] = bit;
		}
