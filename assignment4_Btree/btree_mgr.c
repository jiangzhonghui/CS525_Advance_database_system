...
RC insertKey (BTreeHandle *tree, Value *key, RID rid)
{
    ...
    while (temp != NULL) {
        ...
        if((node_full == 0) && ((*temp).next[max_element] == NULL)) {
...
RC openTreeScan (BTreeHandle *tree, BT_ScanHandle **handle)
{
    ...
    while (temp != NULL){
        ...
        while (i < max_element){
            ...
            i += 1;
			index_total=i*total_element;
        }
        temp = (*temp).next[max_element];//
    }

    int elements[max_element][total_element];
    int count = 0;
    int key[total_element];   
    temp = r_node;
    while (temp != NULL) {
        i = 0;
        while (i < max_element) {
...
{
    check_cond((scan->next[max_element] == NULL) ,{
        return RC_IM_NO_MORE_ENTRIES;
    },
    {
        check_cond((max_element != i_num), {
            n_value = 2;
        },{
            i_num = 0;
